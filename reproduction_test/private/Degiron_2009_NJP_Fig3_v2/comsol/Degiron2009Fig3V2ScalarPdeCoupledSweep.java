...
public class Degiron2009Fig3V2ScalarPdeCoupledSweep {
  private static final double LAMBDA0_UM = 1.55;
  private static final double K0 = 2.0 * Math.PI / (LAMBDA0_UM * 1.0e-6);
  private static Model lastModel = null;

  public static Model run() throws Exception {
    double[] tValues = new double[]{4.8, 5.0, 5.2, 5.3, 5.4, 5.5, 5.6, 5.7, 5.8, 5.9, 6.0, 6.2, 6.4, 6.6, 6.8, 7.0, 7.2, 7.4, 7.6, 7.8, 8.0, 8.2, 8.4, 8.6, 8.8, 9.0, 9.2, 9.4, 9.6, 9.8, 10.0};
    ArrayList<String> rows = new ArrayList<String>();
    for (int i = 0; i < tValues.length; i++) {
      runCase(rows, "coupled", tValues[i], true, true);
    }
    printRows(rows);
    System.out.println("DEGIRON_2009_FIG3_V2_COUPLED_SWEEP_OK rows=" + rows.size() + " method=scalar_tm_hx_pde");
    if (lastModel == null) {
  ...
  private static Model buildModel(String caseName, double tBcb, boolean includeAu, boolean includeSu8) throws Exception {
    ...
    model.mesh("mesh1").feature("size1").set("hmax", "0.09");
    model.mesh("mesh1").feature("size1").set("hmin", "0.006");
    try {
