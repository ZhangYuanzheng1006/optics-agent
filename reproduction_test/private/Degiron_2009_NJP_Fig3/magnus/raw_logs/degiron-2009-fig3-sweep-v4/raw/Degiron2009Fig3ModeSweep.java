...
public class Degiron2009Fig3ModeSweep {
  ...
  private static final String RUN_VARIANT = "sweep";
  private static final boolean TRY_FULL_VECTOR = false;
  ...
  private static ArrayList<double[]> solveFullVectorModeAnalysis(double tBcb) throws Exception {
    ...
    for (int i = 0; i < shifts.length; i++) {
      try {
        return solveFullVectorModeAnalysisOnce(tBcb, shifts[i]);
      } catch (Throwable ex) {
        errors.append("shift=").append(shifts[i]).append(": ").append(clean(ex.toString())).append(" | ");
      }
    }
    throw new RuntimeException("all mode-analysis shifts failed: " + errors.toString());
  }

  private static ArrayList<double[]> solveFullVectorModeAnalysisOnce(double tBcb, String shiftExpr) throws Exception {
    ...
    model.param().set("beta_search", shiftExpr);
  ...
  private static Model ensureOutputModel() throws Exception {
    ...
    lastModel = ModelUtil.create("DegironGeometryOnly");
    lastModel.modelNode().create("comp1");
    lastModel.param().set("lambda0", LAMBDA0_UM + "[um]");
    lastModel.param().set("t_bcb", "5.6[um]");
    lastModel.geom().create("geom1", 2);
    lastModel.geom("geom1").lengthUnit("um");
    rectangle(lastModel, "all", "-12", "-6", "24", "(t_bcb+10)");
    rectangle(lastModel, "si", "-12", "-6", "24", "2");
    rectangle(lastModel, "sio2", "-12", "-4", "24", "4");
    rectangle(lastModel, "bcb", "-12", "0", "24", "t_bcb");
    rectangle(lastModel, "air", "-12", "t_bcb", "24", "4");
    rectangle(lastModel, "au", "-4.6", "3.3", "4.6", "0.036");
    rectangle(lastModel, "su8", "2.5", "3.3", "2.0", "1.5");
    lastModel.geom("geom1").run();
    lastModel.material().create("mat1", "Common", "comp1");
    lastModel.material("mat1").selection().all();
    lastModel.material("mat1").propertyGroup("def").set("relpermittivity", new String[]{
      epsExpr(), "0", "0", "0", epsExpr(), "0", "0", "0", epsExpr()
    });
    return lastModel;
  }
