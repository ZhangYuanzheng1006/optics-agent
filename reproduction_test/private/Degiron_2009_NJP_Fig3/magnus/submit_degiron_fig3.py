...
class DegironCase:
    ...
    max_wait_hours: float = 0.75

    @property
    def remote_input_file(self) -> str:
        return f"{REMOTE_CASE_DIR}/Degiron2009Fig3ModeSweep.java"
...
SMOKE = DegironCase("smoke", "degiron-2009-fig3-smoke-v4", max_wait_hours=0.75)
SWEEP = DegironCase("sweep", "degiron-2009-fig3-sweep-v4", max_wait_hours=6.0)
...
def check_resource_limit(case: DegironCase) -> dict[str, Any]:
...
def run(cmd: list[str], *, timeout: int = 120, check: bool = True) -> subprocess.CompletedProcess[str]:
    result = subprocess.run(cmd, text=True, encoding="utf-8", errors="replace", stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False, timeout=timeout)
...
def stage_case_files(dry_run: bool) -> dict[str, Any]:
    files = [
        CASE_ROOT / "comsol" / "Degiron2009Fig3ModeSweep.java",
        CASE_ROOT / "comsol" / "run_config_smoke.json",
        CASE_ROOT / "comsol" / "run_config_sweep.json",
        CASE_ROOT / "comsol" / "postprocess_degiron_fig3.py",
    ]
    ...
    for file in files:
        run(["scp", str(file), f"{REMOTE_HOST}:{REMOTE_CASE_DIR}/"], timeout=120)
...
def blueprint_args(case: DegironCase) -> dict[str, Any]:
    return {
        "run_mode": "batch_java",
...
def launch_case(case: DegironCase) -> str:
...
def record(case: DegironCase, job_id: str, status: str, action: str, resource_check: dict[str, Any] | None) -> dict[str, Any]:
    return {
        "case": case.name,
        "run_id": case.run_id,
        "job_id": job_id,
...
def submit_or_reuse(case: DegironCase, *, dry_run: bool, force_rerun_failed: bool) -> dict[str, Any]:
...
def poll_interval(case: DegironCase, elapsed: float) -> int:
    if case.name == "smoke":
        return 60 if elapsed < 600 else 180
    return 300 if elapsed < 3600 else 900


def wait_for_case(case: DegironCase, rec: dict[str, Any]) -> dict[str, Any]:
...
def download_run(case: DegironCase, rec: dict[str, Any]) -> dict[str, Any]:
    local_dir = CASE_ROOT / "magnus" / "raw_logs" / case.run_id
    local_dir.parent.mkdir(parents=True, exist_ok=True)
    if local_dir.exists():
        shutil.rmtree(local_dir)
    result = run(["scp", "-r", f"{REMOTE_HOST}:{case.output_dir}", str(local_dir)], timeout=300, check=False)
    rec["download"] = {
...
def postprocess(local_run_dir: Path) -> dict[str, Any]:
    script = CASE_ROOT / "comsol" / "postprocess_degiron_fig3.py"
    result = run([sys.executable, str(script), "--run-dir", str(local_run_dir), "--output-dir", str(CASE_ROOT / "results")], timeout=120, check=False)
    return {
        "returncode": result.returncode,
...
def write_markdown_report(records: list[dict[str, Any]], stage_info: dict[str, Any], path: Path) -> None:
    ...
    for rec in records:
        lines.append(f"| `{rec['case']}` | `{rec.get('job_id','')}` | `{rec.get('status','')}` | `{rec.get('dedupe_action','')}` | `{rec.get('output_dir','')}` |")
...
def job_id_lines(records: list[dict[str, Any]]) -> list[str]:
    lines = ["# Degiron 2009 Fig. 3 Job IDs", ""]
    for rec in records:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stage and submit Degiron 2009 Fig. 3 COMSOL reproduction jobs.")
...
def main() -> int:
    ...
    if not args.sweep_only:
        smoke = submit_or_reuse(SMOKE, dry_run=args.dry_run, force_rerun_failed=args.force_rerun_failed)
        ...
        if not args.dry_run and not args.no_wait:
            ...
            records[-1] = smoke
            if smoke.get("status") == "Success" and not args.no_download:
                records[-1] = download_run(SMOKE, smoke)
                local = CASE_ROOT / "magnus" / "raw_logs" / SMOKE.run_id
                records[-1]["postprocess"] = postprocess(local)
            write_markdown_report(records, stage_info, CASE_ROOT / "magnus" / "submit_log.md")
        if not args.dry_run and smoke.get("status") != "Success" and not args.smoke_only:
            write_markdown_report(records, stage_info, CASE_ROOT / "magnus" / "submit_log.md")
            print(json.dumps(records, ensure_ascii=False, indent=2))
            raise SystemExit("Smoke did not reach Success; sweep was not submitted.")

    if not args.smoke_only:
        sweep = submit_or_reuse(SWEEP, dry_run=args.dry_run, force_rerun_failed=args.force_rerun_failed)
        records.append(sweep)
        write_markdown_report(records, stage_info, CASE_ROOT / "magnus" / "submit_log.md")
        if not args.dry_run and not args.no_wait:
            sweep = wait_for_case(SWEEP, sweep)
            records[-1] = sweep
            if sweep.get("status") == "Success" and not args.no_download:
                records[-1] = download_run(SWEEP, sweep)
                local = CASE_ROOT / "magnus" / "raw_logs" / SWEEP.run_id
                records[-1]["postprocess"] = postprocess(local)
            write_markdown_report(records, stage_info, CASE_ROOT / "magnus" / "submit_log.md")
