...
通过解析（半解析）论文复现，深入理解 Mie 散射的数学结构和物理图像，掌握检验模型正确性的系统方法，实践 SPP 课程学习的内容，为后续 COMSOL 数值计算的验证提供经过解析检验的基础模型和基准数据。
...
根据 SPP 的学习和文献调研，我准备选择以下条件作为检验复现正确性的标准：
...
在 AI agent 的辅助下进行资料搜索、论文阅读、公式推导、代码编写和结果验证。文献查找不只依赖 AI 搜索结果，还会尽量通过 Web of Science 追踪经典文献和高被引代表文献，避免只选近期 arXiv 综述。每篇论文独立实施，流程如下：

1. **读论文**，理解物理问题和理论框架
2. **推导关键公式**，写出从 Maxwell 方程组到最终表达式的完整过程
3. **编程实现**，用 Python（numpy/scipy）将公式转化为可运行的代码
4. **通过检验标准**，逐一验证一、中的 6 条物理约束
5. **与论文对比**，将计算结果叠加到论文原图上检验一致性

全部计算基于特殊函数的数值求值。AI agent 辅助完成但不替代物理判断——所有推导、假设和检验结论由人工确认。如果物理方面有不能解决的困难，会及时向学长学姐请教。

扩展内容：在所有工作成功后，尝试 agent 工作流能否脱离人的引导，独立得出这些内容。

### 交付汇报成果

每篇论文：

- 推导笔记（目前计划是 Markdown + PDF）
- 可复现的计算代码（目前计划是 Python）
- 与论文结果对比的曲线图 (和推导笔记合为正式报告)
- 标注了共振位置和关键参数的数据表 (和推导笔记合为正式报告)
- 每个层次得到的物理结论总结，例如金属小球和介质小球模式结构的差异、共振峰位随尺寸和折射率变化的规律、阵列耦合带来的新共振机制

所有数据统一管理，作为后续 COMSOL 数值计算结果的验证基准。

### 预计时间

- 预计一周左右优先完成单一小球层次的理论建立，包括金属小球和介质小球的 Mie 模式对比，并进行一次汇报，请老师点评指导，检查我的工作方向和细节是否有问题
- 在第一次汇报时根据情况确认后续核壳结构、周期阵列和有效介质部分的完成顺序与时间
...
以下论文分别来自 SPP 课程 PPT 引用、Web of Science 经典引用链和 arXiv 搜索。具体执行时会先完成单一小球（金属小球/介质小球）的理论建立，再进入核壳结构、周期阵列和有效介质等更高层次，避免过快推进导致基础物理结论不清楚。
...
**论文**：优先通过 Web of Science 补充经典代表文献，可参考 García-Etxarri et al. (2011), Kuznetsov et al., "Magnetic light" (Sci. Rep. 2012), Evlyukhin et al., "Demonstration of Magnetic Dipole Resonances of Dielectric Nanospheres" (Nano Lett. 2012), 以及 Kuznetsov et al., "Optically resonant dielectric nanostructures" (Science 2016)。

**摘要**：高折射率介质小球不同于金属小球，内部位移电流可以形成磁偶极、电偶极、磁四极和电四极等多种 Mie 模式，且损耗通常低于金属等离激元结构。

**目的**：在单一金属小球的基础上加入介质小球，比较金属 LSPR 与介质 Mie 共振的物理差异，重点理解介质纳米结构中更丰富的电/磁多极模式。

**物理要点**：磁偶极模式来自球内环形位移电流；共振位置主要由尺寸参数和折射率决定；电偶极与磁偶极的相对强度决定定向散射和 Kerker 条件。
